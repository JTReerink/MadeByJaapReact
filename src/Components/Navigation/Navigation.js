...
const Navigation = () => {
    ...
    const [scrolledSocials, setScrolledSocials] = useState(false);
    const [hiddenSocials, setHiddenSocials] = useState(false);

    useEffect(() => {

        window.addEventListener("scroll", () => {
            if(window.scrollY > 1) setScrolledSocials(true);
            else setScrolledSocials(false);
            
            if (window.scrollY > 100) setHidden(true);
            else setHidden(false);

            if (document.documentElement.scrollHeight - window.scrollY - window.innerHeight < 300) setHiddenSocials(true);
            else setHiddenSocials(false);
        })
    }, []);
    ...
    return (
        <nav className="navigation">
            ...
            {/*             when scrolledSocials but not hiddenSocials (hiddenSocials takes precedence)                         when hiddenSocials                                                                     This is the default*/}
            <div className={scrolledSocials && !hiddenSocials ? "navigation__socialButtons navigation__socialButtons--hidden" : hiddenSocials ? "navigation__socialButtons navigation__socialButtons--hiddenSocials" : "navigation__socialButtons"}>
